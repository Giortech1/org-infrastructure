...
resource "local_file" "cost_control_readme" {
  content  = <<-EOF
# Cost Control Best Practices for ${upper("giortech-dev")}
...
2. **Limited log retention** (7 days)
3. **Log filtering** to exclude health checks and debug logs

## Monitoring Dashboards

Access the cost monitoring dashboards at:
${module.giortech_dev_cost_controls.dashboards.cost_dashboard}

## Alert Notifications

Budget alerts are sent to: ${module.giortech_dev_cost_controls.notification_channel}
...
`${module.giortech_dev_cost_controls.log_metric_name}`
...
```
  EOF
  filename = "../../../docs/giortech-dev-cost-controls.md"
}